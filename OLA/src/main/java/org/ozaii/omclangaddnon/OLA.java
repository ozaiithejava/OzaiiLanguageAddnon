...
import org.ozaii.omclangaddnon.commands.*;
...
import org.ozaii.omclangaddnon.placeholders.ExpercoinsPlaceholderExpansion;
...
public final class OLA extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        getCommand("globalreload").setExecutor(new ReloadCommand(this));
        getCommand("private-block").setExecutor(new PrivateBlockCommand());
        getCommand("private-unblock").setExecutor(new PrivateUnblockCommand());

        getServer().getPluginManager().registerEvents(new ChatDisableListener(config,new MessageUtils(config)), this);

        if (getServer().getPluginManager().getPlugin("PlaceholderAPI") != null) {
            new ExpercoinsPlaceholderExpansion(this,new GlobalMessageFactory(),new PlayerPrivateFactory(),config).register();
        }
